        ...
        for record in values:
            ...
            for field, column in zip(record, columns_record):
                if column != "time":
                    ...
                    _to_write += conj + str(column) + "=" + str(field)
                else:
                    time, date_time=timedateToTimestamp(field)
                    _to_write_time = ' ' + time + '\n'
            if int(time) > int(datetime_start.ljust(len(time),'0')):
                f.write(table_name + _to_write + _to_write_time)
    f.close
...
writeFile(file_name_output, table_names, data, columns, timestamp)
